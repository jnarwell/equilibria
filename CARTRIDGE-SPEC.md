...
- **Vanilla, no deps, no CDN.** Must run by double-clicking `studio.html`.
- **Read params live** every frame from the passed `params` object. The slider
  mutates it in place; don't snapshot values in `create`.
...
- **Palette continuity.** Reuse the thermal→verdant ramp
  (`#1b4d3e · #2a9d8f · #00ff9c · #d4a017 · #ff7b00 · #ff4d00` on `#0a0e0b`)
  so every cartridge reads as part of the same body of work.

## The commission loop

New cartridge = describe it → the agent writes `cartridges/NN-slug.js` to this
contract → add one `<script src>` line to `studio.html` → it appears in the rack.
